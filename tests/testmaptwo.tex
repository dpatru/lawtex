...
\begin{LTXexample}
  ...
  \renewcommand\do[1]{%
    ...
    % \edef\cite{\ltrim #1}%
    \edef\cite{#1}%
    \par cite is (\cite)\par%
    \edef\book{\expandafter\getbook\cite;}\par book is (\book)\par%
    \edef\page{\expandafter\getpage\cite;}\par page is (\page)\par%
    \par before-books is (\books)\par%
    \IfEq{\books}{}{%
    \par before-books is empty(\books)\par%
    \edef\books{\book}\edef\pages{\page}%
    }{% else books is not empty
    \par before-books is not empty (\books)\par%
    \edef\books{\books;\book}%
    \edef\pages{\pages;\page}%
    }%
    \par books is (\books)\par%
    \par pages is (\pages)\par%
    \vspace{5mm}%
    }
